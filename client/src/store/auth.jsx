...
export const AuthProvider = ({ children }) => {
  ...
  const userAuthentication = async () => {
    try {
      const response = await fetch("https://mern-admin-0miv.onrender.com/api/auth/user", {
        method: "GET",
        headers: {
  ...
  const getServices = async () => {
    try {
      const response = await fetch("https://mern-admin-0miv.onrender.com/api/data/service", {
        method: "GET"
      });
      if (response.ok) {
        const data = await response.json();
